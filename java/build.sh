#!/usr/bin/env bash
# Build ghostdroid-server.jar (dexed) from java/PhantomServer.java.
# Port of vendor contrib/android-server/build.sh: javac source/target 8 + d8
# --min-api 26, output build/ghostdroid-server.jar, classes.dex presence check.
# Branding: ghostdroid-server.jar, class com.ghostdroid.server.PhantomServer.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT_DIR="$ROOT/build"
SRC_FILE="$ROOT/PhantomServer.java"
...
"$D8_BIN" \
  ...
  --lib "$ANDROID_JAR" \
  --min-api 26 \
  --output "$OUT_DIR/ghostdroid-server.jar" \
  "$OUT_DIR/classes.jar"

if ! unzip -l "$OUT_DIR/ghostdroid-server.jar" | grep -q "classes.dex"; then
  echo "ghostdroid-server.jar missing classes.dex (d8 failed)" >&2
  exit 1
fi

echo "Built $OUT_DIR/ghostdroid-server.jar"

...
// GhostDroid Android touch server.
// Port of vendor `.devdocs/phantom/contrib/android-server/.../PhantomServer.java`
// (344 lines). Same LE wire protocol on port 27183:
//   0x00 DOWN [slot u8, x i32 LE, y i32 LE], 0x01 MOVE (same),
//   0x02 UP [slot u8], 0x03 CANCEL, 0x7f PING (echoed).
// TouchInjector: 10 slots, MotionEvent.obtain + InputManager.injectInputEvent ASYNC.
// Build: javac + d8, same steps as vendor contrib/android-server/build.sh
// (-> ghostdroid-server.jar).
// IDC ref: `.devdocs/phantom/contrib/waydroid/Vendor_1234_Product_5678.idc`.
public final class PhantomServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("ghostdroid-server starting host=" + bindHost + " port=" + port);
        ...
        try (ServerSocket server = new ServerSocket()) {
            ...
            while (true) {
                try (Socket client = server.accept()) {
                    ...
                    System.out.println("ghostdroid-server client connected");
                    runSession(client.getInputStream(), client.getOutputStream());
                    injector.cancelAll();
                    System.out.println("ghostdroid-server client disconnected");
                } catch (EOFException eof) {
                    injector.cancelAll();
                    System.out.println("ghostdroid-server client closed");
                }
            }
        }
    }
